import {Address, Box, BoxGraph, PointerField} from "@dlm-daw/lib-box"
import {assert, ByteArrayInput, ByteArrayOutput, Option, panic, UUID} from "@dlm-daw/lib-std"
import {DeviceBox, DeviceBoxUtils} from "@dlm-daw/studio-adapters"
import {AudioFileBox, BoxIO} from "@dlm-daw/studio-boxes"
...
export namespace DeviceIO {
    export const exportDevice = (box: Box): ArrayBufferLike => {
        ...
        output.writeString("dlm-daw:device")
    ...
    export const importDevice = (boxGraph: BoxGraph<BoxIO.TypeMap>, buffer: ArrayBufferLike): DeviceBox => {
        ...
        assert(header === "dlm-daw:device", `wrong header: ${header}`)
